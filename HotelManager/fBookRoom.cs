...
namespace HotelManager
{
    public partial class fBookRoom : Form
    {
        ...
        public void InsertBookRoom(int idCustomer, int idRoom, DateTime datecheckin, DateTime datecheckout, DateTime datebookroom)
        {
            BookRoomDAO.Instance.InsertBookRoom(idCustomer, idRoom, datecheckin, datecheckout, datebookroom);
        }
        ...
        private void UpdateSttRoomUsing()
        {
            ...
            if (cbRoom.SelectedItem != null)
            {
                try
                {
                    ...
                    Customer customer = CustomerDAO.Instance.GetInfoByIdCard(txbIDCard.Text);
                    string IDCard = customer.IdCard.ToString();
                    selectedRoom.IdStatusRoom = 1;
                    RoomDAO.Instance.UpdateStatusRoom(selectedRoom.Id, IDCard);
                    RoomDAO.Instance.UpdateIDCustomerBookRoom(selectedRoom.Id, IDCard);
                }
                catch
                {
                    MessageBox.Show("Lỗi!!!!");
        ...
        {
            if (txbIDCardSearch.Text != String.Empty || txbIDCardSearch.Text.Length != 9 || txbIDCardSearch.Text.Length != 12)
            {
                if (IsIdCardExists(txbIDCardSearch.Text))
                    GetInfoByIdCard(txbIDCardSearch.Text);
                else
                    MessageBox.Show("CMND/CCCD không tồn tại.\nVui lòng nhập lại.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }    
        }
        ...
        private void btnBookRoom_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có muốn đặt phòng không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (txbIDCard.Text != String.Empty && txbFullName.Text != String.Empty && txbAddress.Text != String.Empty && txbPhoneNumber.Text != String.Empty && cbNationality.Text != String.Empty)
                {
                    ...
                    InsertBookRoom(CustomerDAO.Instance.GetInfoByIdCard(txbIDCard.Text).Id, (int)RoomDAO.Instance.GetIdRoomByRoomName(cbRoom.Text), dpkDateCheckIn.Value, dpkDateCheckOut.Value, DateTime.Now);
                    ...
                    {
                        this.Hide();
                        fReceiveRoom fReceiveRoom = new fReceiveRoom(GetCurrentIDBookRoom(DateTime.Now.Date), cbRoom.SelectedItem.ToString());
